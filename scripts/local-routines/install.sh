...
# ── 3b. write env.sh so runner.sh has notify vars on manual triggers
# (launchd-fired runs read these from the plist; this file covers the
# `~/insigtrade-routines/runner.sh daily` path you'd run by hand)
{
  echo "# Auto-generated by install.sh — do not edit by hand."
  echo "export NOTIFY_EMAIL=\"${NOTIFY_EMAIL}\""
  if [[ -n "${RESEND_API_KEY:-}" ]]; then
    echo "export RESEND_API_KEY=\"${RESEND_API_KEY}\""
  fi
} >"$ROUTINE_DIR/env.sh"
chmod 600 "$ROUTINE_DIR/env.sh"
echo "✓ Wrote $ROUTINE_DIR/env.sh (mode 600)"
